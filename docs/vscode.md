...
## version 1.26 (2018/8)
1. 新增麵包屑導覽，可用「檢視」功能表開啟，並用快捷鍵 ` Ctrl+Shift+. ` 操控。（可代替「大綱」）
2. 問題面板快速修復：當懸停或選擇問題條目時，會顯示「快速修復燈泡」。可單擊燈泡或打開問題條目的上下文菜單來應用快速修復。
3. 現可複製檔案的相對路徑（可用快顯功能表或快捷鍵）  
	> 複製絕對路徑：` Shift+Alt+C `。  
	> 複製檔案的相對路徑：` Ctrl+K Ctrl+Alt+C `
